...
// validates object using custom GET api.
func identityGetValidateObject(c *s3.S3, input *s3.GetObjectInput, verboseValidate bool) (output *s3.GetObjectOutput, err error) {
	req, out := c.GetObjectRequest(input)
	output = out
	req.HTTPRequest.Header.Set("Accept-Encoding", "identity")
	req.HTTPRequest.URL.RawQuery = "x-ntap-sg-validate"

	err = req.Send()
	if err == nil && req.HTTPResponse.Body != nil {

		buffer := make([]byte, req.HTTPResponse.ContentLength)
		req.HTTPResponse.Body.Read(buffer)

		var verbose = verboseValidate
		// To support usage where we just want the xml responses for failure cases,
		// such as for testing; only dump the respone bodies when there is a failure.
		if !verbose {
			type Result struct {
				XMLName xml.Name `xml:"ValidationResult"`
				Verify  string   `xml:"Verify"`
				IlmEval string   `xml:"IlmEval"`
			}
			v := Result{}
			err := xml.Unmarshal([]byte(buffer), &v)
			if err == nil {
				verbose = v.IlmEval != "Pass" || v.Verify != "Pass"
			} else {
				fmt.Printf("error: %v\n", err)
...
func runtest(args parameters) (float64, bool) {
			...
			for j := 0; uint64(j) <= runNum; j++ {
				...
				for repcount := 0; repcount < args.repeat; repcount++ {
					...
					case args.optype == "validate":
						...
						r.count++
						start = time.Now()
						_, err = identityGetValidateObject(svc, params, args.verboseValidate)
	...
	if args.optype != "validate" || args.verboseValidate {
...
func main() {
	optypes := []string{"put", "multipartput", "get", "puttagging", "updatemeta", "validate", "randget", "delete", "options", "head", "putget", "putget9010r"}
	...
	var verboseValidate = flag.Bool("verbosevalidate", true, "When true, return results for all objects from validate operation. When false, only for failures and hides performance results")
	...
	var verify = flag.Bool("verify", false, "Verify the retrieved data on a get operation - this is different from the validate operation")
	...
	args := parameters{
		...
		verboseValidate:   *verboseValidate,
		overwrite:         *overwrite,
		retries:           *retries,
